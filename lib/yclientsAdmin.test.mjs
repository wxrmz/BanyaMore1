...
import { stripClosedAdminReport } from './adminDashboardAccess.ts';
...
test('daily report follows the TZ formulas and adds KPP discounts to the KPP expense article', () => {
  ...
  assert.equal(report.income, 19_000);
  ...
  assert.equal(report.surrendered, 8_900);
...
test('free-window text starts at the first actually bookable slot', () => {
  const text = buildFreeWindowsFromAvailability([{
    title: 'Малая баня',
    days: [{
      date: '2026-08-25',
...
test('closed administrator response strips every report aggregate but keeps operational data', () => {
  const records = [{ id: 41, client: { name: 'Запись остаётся' } }];
  const safe = stripClosedAdminReport({
    ok: true,
    date: '2026-09-14',
    ...
    baths: [{
      ...
      revenue: 50_000,
      prepayments: 10_000,
      kitchenRevenue: 5_000,
      kitchenOrders: [{ title: 'Блюдо', quantity: 1 }],
      kppChecks: 400,
      discounts: 1_000,
      records,
    }],
    ...
    beer: [{ id: 4 }],
    drinks: [{ id: 5 }],
    stocks: [{ id: 'stock-1', title: 'Текущий остаток', stock: 7 }],
    dataHealth: { state: 'complete', sources: [], issues: [] },
    copyText: {
      freeWindows: 'Свободно',
      occupiedTimes: '10:00 - 12:00',
      occupiedBaths: 'Малая баня',
    },
    generatedAt: '2026-09-14T02:00:00.000Z',
  });

  for (const field of ['report', 'kitchen', 'additionalServices', 'goods', 'beer', 'drinks']) {
    assert.equal(field in safe, false, `${field} must be absent`);
  }
  assert.deepEqual(safe.baths, [{
    id: 'small',
    title: 'Малая баня',
    shortTitle: 'МБ',
    records,
  }]);
  assert.deepEqual(safe.stocks, [{ id: 'stock-1', title: 'Текущий остаток', stock: 7 }]);
  assert.equal(safe.copyText.freeWindows, 'Свободно');
});
